...
function loadSettings(): { themeMode: ThemeMode; soundEnabled: boolean; expandedUI: boolean; micDeviceId: string | null } {
  try {
  ...
  return { themeMode: 'dark', soundEnabled: true, expandedUI: false, micDeviceId: null }
}

function saveSettings(s: { themeMode: ThemeMode; soundEnabled: boolean; expandedUI: boolean; micDeviceId: string | null }): void {
...
export const useThemeStore = create<ThemeState>((set, get) => ({
  ...
  setThemeMode: (mode) => {
    ...
    applyTheme(resolved)
    saveSettings({ themeMode: mode, soundEnabled: get().soundEnabled, expandedUI: get().expandedUI, micDeviceId: get().micDeviceId })
  },
  setSoundEnabled: (enabled) => {
    set({ soundEnabled: enabled })
    saveSettings({ themeMode: get().themeMode, soundEnabled: enabled, expandedUI: get().expandedUI, micDeviceId: get().micDeviceId })
  },
  setExpandedUI: (expanded) => {
    set({ expandedUI: expanded })
    saveSettings({ themeMode: get().themeMode, soundEnabled: get().soundEnabled, expandedUI: expanded, micDeviceId: get().micDeviceId })
  },
  setMicDeviceId: (id) => {
    set({ micDeviceId: id })
    saveSettings({ themeMode: get().themeMode, soundEnabled: get().soundEnabled, expandedUI: get().expandedUI, micDeviceId: id })
  },
